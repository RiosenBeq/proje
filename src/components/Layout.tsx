...
export default function Layout() {
  return (
    <div className="min-h-screen flex flex-col">
      {/* Top Navigation Bar */}
      <header className="fixed top-0 w-full flex justify-between items-center px-8 py-4 bg-[#5a6780]/82 backdrop-blur-xl z-50 border-b border-white/20 shadow-[0_20px_40px_rgba(40,50,68,0.28)]">
        ...
        <nav className="hidden md:flex items-center gap-8 lg:gap-12 font-['Space_Grotesk'] tracking-widest uppercase text-sm">
          <NavLink to="/" className={({isActive}) => isActive ? "text-[#f06b8e] border-b-2 border-[#f06b8e] pb-1" : "text-white/60 hover:text-white transition-colors"}>Ana Sayfa</NavLink>
          <NavLink to="/hizmetler" className={({isActive}) => isActive ? "text-[#f06b8e] border-b-2 border-[#f06b8e] pb-1" : "text-white/60 hover:text-white transition-colors"}>Hizmetler</NavLink>
          <NavLink to="/portfolyo" className={({isActive}) => isActive ? "text-[#f06b8e] border-b-2 border-[#f06b8e] pb-1" : "text-white/60 hover:text-white transition-colors"}>Portfolyo</NavLink>
          <NavLink to="/iletisim" className={({isActive}) => isActive ? "text-[#f06b8e] border-b-2 border-[#f06b8e] pb-1" : "text-white/60 hover:text-white transition-colors"}>İletişim</NavLink>
        </nav>
      ...
      <aside className="fixed right-0 top-0 h-full flex flex-col items-center py-24 z-40 bg-[#495570]/88 backdrop-blur-2xl border-l border-white/20 w-20 hidden lg:flex font-['Space_Grotesk'] text-[10px] font-light uppercase">
      ...
      {/* Footer */}
      <footer className="w-full bg-[#3f4a5f] py-12 px-8 lg:px-24 border-t border-white/20 flex flex-col md:flex-row justify-between items-start gap-8 mt-auto z-10 relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-12">
          <div className="flex flex-col gap-4">
            ...
            <Link to="/" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">Biz Kimiz?</Link>
            <Link to="/" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">Çözüm Ortakları</Link>
            <Link to="/" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">Kariyer</Link>
          </div>
          <div className="flex flex-col gap-4">
            <span className="text-[10px] font-label text-primary uppercase tracking-widest">Hizmetler</span>
            <Link to="/hizmetler" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">Stüdyo Akustiği</Link>
            <Link to="/hizmetler" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">PA Sistemleri</Link>
            <Link to="/hizmetler" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">Kalibrasyon</Link>
          </div>
          <div className="flex flex-col gap-4 col-span-2 md:col-span-1">
            <span className="text-[10px] font-label text-primary uppercase tracking-widest">Yasal</span>
            <Link to="/" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">Gizlilik Politikası</Link>
            <Link to="/" className="font-body text-xs text-white/40 hover:text-[#f06b8e] transition-colors">KVKK</Link>
          </div>
        </div>
      </footer>
